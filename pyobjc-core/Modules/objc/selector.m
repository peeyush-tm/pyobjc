...
{
	if (value == NULL) {
		PyErr_Format(PyExc_TypeError, "cannot delete method %s on an instance",
                        sel_getName(PyObjCSelector_GetSelector(self)));
	} else {
		/* Dummy, setattr of the class deals with this */
		PyErr_Format(PyExc_TypeError, "cannot change a method %s on an instance",
                        sel_getName(PyObjCSelector_GetSelector(self)));
	}
	return -1;
}
